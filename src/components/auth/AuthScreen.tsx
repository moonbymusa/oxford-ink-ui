...
const field =
  "w-full rounded-[10px] border border-input bg-input/40 px-10 py-2.5 text-sm outline-none transition-shadow placeholder:text-muted-foreground/70 focus:border-brand focus:shadow-[var(--shadow-brand)]";
...
export function AuthScreen({ initialTab = "signin" }: { initialTab?: "signin" | "signup" }) {
  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-[40fr_60fr]">
      {/* ── Brand panel ─────────────────────────────────────── */}
      <section className="relative flex flex-col justify-between overflow-hidden border-r border-border p-8 md:p-12">
        <div className="pointer-events-none absolute inset-0 bg-gradient-brand opacity-[0.14]" />
        <div className="relative">
          <div className="flex items-center gap-3">
            <div className="grid size-11 place-items-center rounded-2xl bg-gradient-brand shadow-[var(--shadow-brand)]">
              <ScanEye size={22} className="text-primary-foreground" />
            </div>
            <div>
              <p className="font-display text-lg font-extrabold tracking-tight">ScriptGrade</p>
          ...
          <h1 className="mt-14 max-w-md font-display text-4xl leading-[1.1] font-extrabold tracking-tight md:text-[3.25rem]">
        ...
        <div className="relative mt-10 flex flex-wrap items-center gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-vision/40 bg-vision/10 px-3 py-1.5 text-[0.6875rem] font-semibold text-vision">
            <Sparkles size={11} /> Alibaba Cloud · Qwen3.8-Max Powered
          </span>
          <span className="inline-flex items-center gap-1.5 rounded-full border border-pass/40 bg-pass/10 px-3 py-1.5 text-[0.6875rem] font-semibold text-pass">
            <ShieldCheck size={11} /> JWT · RBAC secured
          </span>
          <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-muted/50 px-3 py-1.5 text-[0.6875rem] font-semibold text-muted-foreground">
      ...
      <section className="flex items-center justify-center p-6 md:p-12">
        <div className="glass w-full max-w-[440px] rounded-3xl p-6 md:p-8 spring-in">
          <div role="tablist" className="mb-6 grid grid-cols-2 gap-1 rounded-xl border border-border bg-muted/40 p-1">
            {(["signin", "signup"] as const).map((t) => (
              <button
                ...
                className={cn(
                  ...
                  tab === t
                    ? "bg-gradient-brand text-primary-foreground shadow-[var(--shadow-md)]"
          ...
          {tab === "signin" ? (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <button className="text-brand-light hover:underline">Forgot?</button>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-[10px] border border-input bg-input/40 px-3 py-2.5 text-sm outline-none focus:border-brand focus:shadow-[var(--shadow-brand)]"
                >
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {[
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 rounded-xl border border-border bg-muted/30 py-2.5 text-xs font-semibold text-muted-foreground transition-colors hover:border-brand/50 hover:text-foreground"
              >
                <Icon size={14} /> {label}
              </button>
            ))}
...
function PrimaryButton({
  ...
  return (
    <button
      onClick={onClick}
      disabled={loading}
      className="magnetic flex w-full items-center justify-center gap-2 rounded-xl border border-brand/50 bg-brand/20 py-3 text-sm font-bold text-foreground disabled:opacity-60"
    >
