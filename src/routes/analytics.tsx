...
function heatTone(pct: number) {
  if (pct >= 85) return "bg-pass/25 text-pass border-pass/40";
  if (pct >= 70) return "bg-vision/25 text-vision border-vision/40";
  if (pct >= 55) return "bg-warn/25 text-warn border-warn/40";
  return "bg-alert/25 text-alert border-alert/40";
}
...
function AnalyticsPage() {
  ...
  return (
      ...
      actions={
        <>
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-3.5 py-2.5 text-sm font-semibold transition-colors hover:border-brand/50"
          >
            <FileSpreadsheet size={15} className="text-pass" /> Export CSV
          </button>
          <button
    ...
    >
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          <div key={k.label} className="glass rounded-2xl p-5">
            ...
            <p
              className="mono-token mt-2 text-2xl font-bold"
      ...
      {/* Heatmap */}
      <section className="glass mt-5 overflow-x-auto rounded-2xl p-5">
        <h2 className="text-base font-semibold">Class Performance Heatmap</h2>
        <p className="mono-token text-[0.6875rem] text-muted-foreground">
        ...
        <table className="mt-4 w-full min-w-[620px] text-left text-xs">
          <thead>
            <tr>
              <th className="px-2 py-2 font-semibold text-muted-foreground">Group</th>
              {CONCEPT_TRENDS.map((c) => (
                <th key={c.concept} className="px-2 py-2 font-semibold text-muted-foreground">
                  {c.concept}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {heatmap.map((row) => (
              <tr key={row.group}>
                <td className="mono-token px-2 py-1.5 font-semibold">{row.group}</td>
                {row.cells.map((cell) => (
                  <td key={cell.concept} className="px-1 py-1.5">
                    <div
                      className={cn(
                        "mono-token rounded-lg border px-2 py-2 text-center font-semibold",
                        heatTone(cell.pct),
                      )}
                    >
                      {cell.pct}%
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
        <section className="glass rounded-2xl p-5">
          <h2 className="text-base font-semibold">Common Error Clusters</h2>
          <p className="mono-token text-[0.6875rem] text-muted-foreground">
        ...
        <section className="glass rounded-2xl p-5">
          <h2 className="text-base font-semibold">Error Share</h2>
          ...
          <button
            onClick={() => exportFile("csv")}
            className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-border py-2.5 text-xs font-semibold transition-colors hover:border-brand/50"
          >
